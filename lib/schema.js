...
const { iter, Enum, Flags, Int64, Uint64, Money } = require('@metarhia/common');
...
const classifyDomains = Symbol('classifyDomains');
...
class Metaschema {
    ...
    const factory = (...args) => {
      ...
      for (let i = 0; i < len; i++) {
        ...
        if (typeof def === 'function') {
          ...
          if (def.category) value = this.createInstance(def.category, value);
  ...
  [classifyDomains]() {
    for (const definition of this.domains.values()) {
